...
export async function getShoppingListsByUser(req: ExpressRequest) {
	...
	const { data, error } = await supabase.from(participantTable).select(
		`shoppingList: shopping_list_id (${shoppingListSelect})`,
	).range(from, to).range(0, 3, { referencedTable: 'shopping_list_id' }).eq(
		'user_id',
		user?.id,
	).order('created_at', {
		ascending: false,
	...
	const shoppingLists = data?.map((item) => {
		return {
			...item.shoppingList,
			items: generateFakeShoppingItemData(10),
		};
	});
